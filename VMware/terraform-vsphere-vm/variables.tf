#######################################################################
# Common variables for both ovf and local template VM provisioning
#######################################################################
variable "vsphere_deployment" {
  description = "If true, deploy to VMWare envirnoment."
  type        = bool
  default     = false
}
...
variable "instance_count" {
  description = "number of instances you want deploy."
  default     = 1
}
...
variable "ram_size_gb" {
  ...
  default     = 4
  validation {
    condition     = var.ram_size_gb < 64
    error_message = "AD has 15 characters for machine name. The node name should less than 11 characters."
  }
}
...
variable "vapp_container" {
  description = "vapp container name"
  type        = string
  default     = null
}
...
variable "vm_os" {
  description = "Type of VM OS. Could be either Windows or Linux"
  type        = string
}
...
variable "node_name" {
  description = "The node name prefix for the VMs"
  type        = string
  default     = ""
  validation {
    condition     = length(var.node_name) < 11
    error_message = "AD has 15 characters for machine name. The node name should less than 11 characters."
  }
}

variable "domain_suffix" {
  description = "Domain name append to node name."
  type        = string
  default     = null
}
...
variable "vsphere_hosts" {
  description = "List of vsphere hosts to launch VMs."
  type        = list(string)
  default     = []
}

variable "server_initial_index" {
  description = "Starting index number that will be append to server name"
  type        = number
  default     = 0
}
...
variable "lb_pool" {
  description = "Load balancer pool to add to."
  type        = string
  default     = ""
}

variable "ssh_user" {
  description = "ssh user to login to the VM."
  type        = string
  default     = null
}

variable "vm_ssh_key" {
  # We need the path to the private key here. We assume the public key is just append .pub to private key.
  # 
  description = "Path to the ssh key pair for ssh into the VM. e.g. ~/.ssh/id_rsa.pem"
  type        = string
  default     = null
}

#######################################################################
# Variables for remote ovf VM provisioning only
#######################################################################
variable "remote_ovf_url" {
  description = "URL for remove ovf template"
  type        = string
  default     = null
}
...
#######################################################################
# Variables for local template VM provisioning only
#######################################################################
variable "local_template_id" {
  description = "Local VM UUID when provision new VMs from local VM copy"
  type        = string
  default     = null
}

variable "local_template_name" {
  description = "Local VM name when provision new VMs from local VM copy"
  type        = string
  default     = null
}
